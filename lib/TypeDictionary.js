...
class TypeDictionary {
  ...
  add(Type, ...aliases) {
    ...
    if (isString(Type) && !Type.trim()) name = Type;else if (isConstructor(Type)) {
      constructor = Type;
      name = Type.name;
    } else if (isNull(Type)) {
      constructor = Type;
    ...
    if (aliases.some(alias => isInvalidString(alias))) throw TypeError(`invalid arguments (${aliases.find(alias => !isInvalidString(alias))})`);
    name = name.trim();
    symbol = Symbol.for(name);
    lowercase = name.toLowerCase();
    if (this.has(symbol, name, lowercase, Type, ...aliases)) throw Error(`Already has type (${name})`); // mark the Type;

    if (constructor) {
      Object.defineProperty(Type.prototype, 'class', assign(description, {
        value: Type
      })); // Object.defineProperty(Type, 'sign', assign(description, {value: symbol}));

      if (constructor.toJSON) aliases.push(constructor.toJSON());else Object.defineProperty(Type, 'toJSON', assign(description, {
        value: toJSON
      }));
    }
  ...
  delete(type) {
    ...
    let l = this[$PROPS].length,
        start = this.indexOf(type) * l,
        end = start + l,
        s = this.symbol(type);
    this[$STORE] = this[$STORE].slice(0, start).concat(this[$STORE].slice(end));

    for (let k in this[$ALIAS]) {
      if (this[$ALIAS].hasOwnProperty(k) && this[$ALIAS][k] === s) delete this[$ALIAS][k];
    }
